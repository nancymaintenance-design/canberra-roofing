...
test('contact form owns accessible error presentation and submission control', async () => {
  ...
  assert.match(source, /if\(firstInvalid\)\{\(form\.elements\.namedItem\(firstInvalid\)/);
  assert.ok(source.indexOf('if(firstInvalid)') < source.indexOf('save({leads:'), 'invalid submission exits before save');
...
test('trims required values and rejects non-canonical selections or non-boolean privacy', () => {
  ...
  assert.equal(validateEnquiry({ ...validEnquiry(), privacy: 1 }).privacy, 'Acknowledge the local demonstration notice to continue.');
});
...
test('Contact and Areas consume canonical exports while serviceSeed retains its page copy', async () => {
  ...
  assert.match(source, /AREA_OPTIONS\.map\(area/);
  assert.match(source, /SERVICE_TITLES\.map\(title/);
