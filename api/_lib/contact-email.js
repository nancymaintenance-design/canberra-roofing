...
function headerValue(value) {
  return String(value ?? '').replaceAll(String.fromCharCode(10), ' ').replaceAll(String.fromCharCode(13), ' ').trim();
}
...
function htmlBody(fields, requestId, timestamp) {
  const rows = [
    ['Service', fields.service], ['Area', fields.area], ['Name', fields.name], ['Email', fields.email],
    ['Phone', fields.phone], ['Address', fields.address || 'Not provided'], ['Message', fields.message],
    ['Submitted UTC', timestamp], ['Request ID', requestId],
  ];
  ...
  return {
    async sendEnquiry({ fields, photo, requestId }) {
      ...
      const message = {
        to: config.to,
        from: config.from,
        reply_to: headerValue(fields.email),
        subject: `Website enquiry: ${headerValue(fields.service)} - ${headerValue(fields.area)}`,
        text: textBody(fields, requestId, timestamp),
        html: htmlBody(fields, requestId, timestamp),
        ...(photo ? { attachments: [{ filename: photo.filename, content: photo.buffer.toString('base64'), content_type: photo.mimetype }] } : {}),
      };
      let response;
      try {
